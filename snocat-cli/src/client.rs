...
pub async fn client_main(config: ClientArgs) -> Result<()> {
  ...
  let (tunnel, incoming) = {
    ...
    let (mut tunnel, mut incoming) = from_quinn_endpoint(connection, TunnelSide::Connect);
    // Authenticate tunnel
    let authenticator = SimpleAckAuthenticationHandler::new();
    let fake_shutdown_trigger = triggered::trigger();
    ::snocat::common::authentication::perform_authentication(
      &authenticator,
      &mut tunnel,
      &mut incoming,
  ...
  tracing::info!(remote = ?tunnel.addr(), "connected");
  ...
  tracing::debug!("Setting up stream handling...");
  {
    ...
    incoming
      .streams()
      .map_err(|e| e.into())
      .try_for_each_concurrent(None, |stream: TunnelIncomingType| {
...
#[tracing::instrument(skip(stream, build_proxy_connection), err)]
