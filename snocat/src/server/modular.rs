  ...
  Self: 'static,
{
  ...
  pub fn handle_request<TProtocolClient: Client + Send + Sync + 'static>(
    self: Arc<Self>,
    ...
    self
      .handle_request_dynamic(request)
  ...
  pub fn handle_request_dynamic(
    self: Arc<Self>,
    request: Request,
  ) -> BoxFuture<'static, Result<Response, RequestHandlingError>> {
    let router = Arc::clone(&self.router);
    let tunnel_registry: Arc<dyn TunnelRegistry + Send + Sync> = Arc::clone(&self.tunnel_registry);
    async move {
